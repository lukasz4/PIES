...
static volatile unsigned int ledValues[] = {0x31, 0xF3, 0x61, 0x49, 0xFF}; //P //I //E //S
#define SHUTDOWN 10
...
static volatile uint8_t displays[] = { LED3, LED2, LED1, LED0 };
...
{
	Board_LED_Set(1, true); //green led during error
	while (1) {}
}
...
static void sendI2CMaster(uint16_t AddressI2C, bool ledStateOut, uint8_t regAddr)
{
	...
	SendData[index++] = (uint8_t) displays[displayNumber]; //displays table
...
int main(void)
{
	...
	while (1) {
		...
		while(1)
		{
			for(int ix=0; ix<4;ix++) //ix is index
			{
			showNumberOnDisplay(ix,ix); //show first number from table on first led display on table and next increment
			showNumberOnDisplay(4,ix); //4 in table is 0xFF - shutdown 7seg
			}
		//	showNumberOnDisplay(0,0);
		//	showNumberOnDisplay(1,1);
		//	showNumberOnDisplay(2,2);
		//	showNumberOnDisplay(3,3);
		}
#endif
		/* Sleep until a state change occurs in SysTick */
